...
export default class SearchScreen extends Component {
  ...
  fetchMoreTransactions = async text =>{
    ...
    if(enteredText[0] === "b"){
      const query = await db.collection("transactions").where("book_id","==", text )
      .startAfter(lastVisibleTransaction).limit(10).get()
      query.docs.map(doc => {
        this.setState({
          allTransactions: [...this.state.allTransactions,doc.data()],
          lastVisibleTransaction: doc
      ...
      const query = await db.collection("transactions").where("student_id","==", text )
      .startAfter(lastVisibleTransaction).limit(10).get()
      query.docs.map(doc => {
        this.setState({
          allTransactions: [...this.state.allTransactions,doc.data()],
